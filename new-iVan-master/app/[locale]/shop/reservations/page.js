...
export default function ServiceReservationsPage() {
  ...
  // Get service config from context or default
  const serviceConfig = contextConfig || getServiceConfig(shopType || "shop");

  useEffect(() => {
    if (status === "loading") return;

    if (!session || (session.user.role !== "shop-owner" && session.user.role !== "restaurant")) {
      router.push("/login");
      return;
    }

    if (shopType && serviceConfig.hasReservations) {
      fetchReservations();
    }
  }, [session, status, router, shopType, serviceConfig]);

  const fetchReservations = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(`/api/reservations?type=${shopType}`, {
        headers: {
    ...
    return (
      <div className="pagetitle">
        ...
        <div className="alert alert-info">
          Reservations are not available for this service type.
        </div>
      </div>
    );
  }
  ...
  if (isLoading) {
    return (
      <div className="pagetitle">
        <h1>{serviceConfig.reservationsLabel || "Reservations"}</h1>
        <div
      ...
      <div className="pagetitle">
        ...
        <nav>
          <ol className="breadcrumb">
            ...
            <li className="breadcrumb-item active">{serviceConfig.reservationsLabel || "Reservations"}</li>
          </ol>
        </nav>
      </div>
            ...
            <div className="feature-box bg-white p-4 d-flex gap-2">
              <div className="feature-details">
                <span className="feature-title fw-bold d-block">
                  Total {serviceConfig.reservationsLabel || "Reservations"}
                </span>
          ...
          <div className="col-lg-12">
            <div className="card all_resveration_card">
              <div className="card-body">
                <h5 className="card-title">All {serviceConfig.reservationsLabel || "Reservations"}</h5>
                        ...
                        <tr>
                          <td colSpan="6" className="text-center py-4">
                            ...
                            <p className="text-muted mt-2">
                              {filter === "all"
                                ? `You don't have any ${serviceConfig.reservationsLabel?.toLowerCase() || "reservations"} yet.`
                                : `You don't have any ${filter} ${serviceConfig.reservationsLabel?.toLowerCase() || "reservations"}.`}
                            </p>
                          </td>
                        </tr>
                      ) : (
                                ...
                                <Dropdown align="end">
                                  ...
                                  <Dropdown.Menu className="rounded shadow">
                                    <Dropdown.Item
                                      onClick={() =>
                                        router.push(
                                          `/shop/reservations/reservation/${reservation.id}`
                                        )
                                      }
                                    >
                                      View Details
