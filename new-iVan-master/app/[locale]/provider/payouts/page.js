...
export default function ProviderPayoutPage() {
    const t = useTranslations("ProviderPages.payouts");
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 transition-all duration-300">
                        {payouts.map(({ title, price, id, completedAt, updatedAt, type, payTo }) => {
                            ...
                            return (
                                ...
                                >
                                    <h4
                                        className={`font-semibold text-[20px] line-clamp-2 ${type === "job" ? "cursor-pointer" : ""}`}
                                        onClick={() => type === "job" && handlePayoutClick(id, type)}
                                    >
                                        {title}
                                    </h4>
