...
const SECTION_ORDER = { ordering: 0, reservation: 1, booking: 2 };
...
/**
 * Public homepage service tiles (Book Now, Services, On Demand ordering).
 * Only returns rows with `homeSection` set and `isActive: true`.
 */
export async function GET(request) {
  try {
    ...
    for (const rawRow of rows) {
      ...
      if (!passesHomepageLocationFilter(row.locationFilter, selectedLocation)) {
        continue;
      }
    ...
    const catalog = [...ordering, ...reservation, ...booking];
    ...
    return NextResponse.json(
      {
        ...
        booking,
        catalog,
        _meta: { sectionOrder: SECTION_ORDER },
      },
      { status: 200 }
    );
  } catch (error) {
    // Migration-safe fallback: if homepage columns are not applied yet,
    // return an empty catalog so UI falls back to static helper services.
    if (error?.code === "P2022") {
      return NextResponse.json(
        {
          ordering: [],
          ...
          booking: [],
          catalog: [],
          _meta: { sectionOrder: SECTION_ORDER, migrationPending: true },
        },
        { status: 200 }
      );
    }
    console.error("public/home-services:", error);
    return NextResponse.json(
      {
